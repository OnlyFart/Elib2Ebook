...
internal static class ProgramCustom {
    ...
    private static async Task Main(string[] args) {
        ...
        await parserResult
            ...
            .WithParsedAsync(async options => {
                ...
                logger.LogInformation( $"Using proxy: {options.Proxy}" );
                logger.LogInformation( $"Using flare: {options.Flare}" );

                if( GetterProvider.IsVolumable( getter ) && options.SplitVolumes )
                {
                ...
                else if( GetterProvider.IsChapterable( getter ) && options.SplitChapters )
                {
                    foreach (var url in options.Url) {
                        try {
                            var originalBookNamePattern = options.BookNamePattern;
