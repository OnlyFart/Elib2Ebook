...
using Elib2Ebook.ExternalServices.AuthorToday.Types;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.AuthorToday;
...
public class AuthorTodayGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var response = await Config.Client.SendAsync(
            GetDefaultMessage(
                ApiUrl.MakeRelativeUri("/v1/account/login-by-password"),
                _apiUrl,
                JsonContent.Create(
                    new
                    {
                        Config.Options.Login, Config.Options.Password
                    })));
    ...
    private async Task FillAdditional(Book book, BookDetails details)
    {
        if (!Config.Options.HasAdditionalType(AdditionalTypeEnum.Images) || details.GalleryImages == null || details.GalleryImages.Length == 0)
        {
            return;
        }
    ...
    private async Task<BookDetails> GetBookDetails(string bookId)
    {
        ...
        return await response.Content.ReadFromJsonAsync<BookDetails>();
    }

    private Author GetAuthor(BookDetails book)
    {
        return new Author(book.AuthorFio, SystemUrl.MakeRelativeUri($"/u/{book.AuthorUserName}/works"));
    }

    private IEnumerable<Author> GetCoAuthors(BookDetails book)
    {
    ...
    private Seria GetSeria(BookDetails book)
    {
    ...
    private Task<TempFile> GetCover(BookDetails book)
    {
    ...
    private async Task<IEnumerable<Chapter>> FillChapters(BookDetails book)
    {
    ...
    private async Task<IEnumerable<AuthorTodayChapter>> GetChapters(BookDetails book)
    {
