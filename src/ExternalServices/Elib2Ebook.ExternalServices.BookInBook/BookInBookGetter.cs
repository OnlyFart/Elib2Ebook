...
namespace Elib2Ebook.ExternalServices.BookInBook;
...
public class BookInBookGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        var urlChapters = doc.QuerySelectorAll("a.chapters-form__chapter").Select(a => new UrlChapter(url.MakeRelativeUri(a.Attributes["href"].Value), a.GetText().ReplaceNewLine()))
            .ToList();
