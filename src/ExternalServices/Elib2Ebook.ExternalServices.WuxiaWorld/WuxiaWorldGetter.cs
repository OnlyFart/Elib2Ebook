...
using Elib2Ebook.ExternalServices.WuxiaWorld.Types.WuxiaWorld;
using HtmlAgilityPack;
using HtmlAgilityPack.CssSelectors.NetCore;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.WuxiaWorld;
...
public class WuxiaWorldGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        {
            Cover = await GetCover(doc, url), Chapters = await FillChapters(doc, url), Title = doc.GetTextBySelector("h1"), Author = new Author("WuxiaWorld"),
        };
    ...
    private async Task<IEnumerable<UrlChapter>> GetToc(HtmlDocument doc, Uri url)
    {
        ...
        foreach (var span in doc.QuerySelectorAll("ul.myUL span.caret[data-id]"))
        {
            ...
            var payload = new FormUrlEncodedContent(
                ...
                {
                    ["cat_id"] = catId, ["offset"] = offset.ToString()
                });
    ...
    private async Task<HtmlDocument> GetSafety(Uri url)
    {
        ...
        while (response == null || response.StatusCode == HttpStatusCode.ServiceUnavailable)
        {
