...
using Elib2Ebook.ExternalServices.StrokiMts.Types;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.StrokiMts;
...
public class StrokiMtsGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        var detailBook = details?.Items.FirstOrDefault(d => d.TextBook != null)?.TextBook ??
                         details?.Items.FirstOrDefault(d => d.AudioBook != null)?.AudioBook ??
                         details?.Items.FirstOrDefault(d => d.ComicBook != null)?.ComicBook ??
                         details?.Items.FirstOrDefault(d => d.PressBook != null)?.PressBook ??
                         await GetSerial(id);
    ...
    private static IEnumerable<Chapter> FillChaptersFromImages(StrokiMtsBookItem book, IEnumerable<TempFile> images)
    {
        ...
        {
            Images = images, Content = sb.ToString(), Title = book.Title,
        };
    ...
    private async Task<List<TempFile>> GetAudio(StrokiMtsApiMultiResponse details)
    {
        ...
        var detail = details?.Items.FirstOrDefault(d => d.AudioBook != null);
        if (detail == null)
        {
            return result;
        }
    ...
    private Author GetAuthor(StrokiMtsBookItem book)
    {
        ...
        return author == null ? new Author("Stroki") : new Author(author.Name, SystemUrl.MakeRelativeUri(author.FriendlyUrl));
    }
