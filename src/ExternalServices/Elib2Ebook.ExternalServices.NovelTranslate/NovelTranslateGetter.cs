...
namespace Elib2Ebook.ExternalServices.NovelTranslate;
...
public class NovelTranslateGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private static Author GetAuthor(HtmlDocument doc, Uri uri)
    {
        ...
        return a != null ? new Author(a.GetText(), uri.MakeRelativeUri(a.Attributes["href"].Value)) : new Author("NovelTranslate");
    }
}
