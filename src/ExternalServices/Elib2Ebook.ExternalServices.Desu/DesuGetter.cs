...
namespace Elib2Ebook.ExternalServices.Desu;
...
public class DesuGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private Author GetAuthor(HtmlDocument doc)
    {
        ...
        return a == null ? new Author("Desu") : new Author(a.GetText(), SystemUrl.MakeRelativeUri(a.Attributes["href"].Value));
    }
    ...
    private async Task<HtmlDocument> GetChapter(UrlChapter urlChapter)
    {
        ...
        var images = Regex.Match(doc.ParsedText, @"Reader\.init(.*?)(?<data>\[\[(.*?)\]\]),", RegexOptions.Singleline).Groups["data"].Value.Replace("'", "\"")
