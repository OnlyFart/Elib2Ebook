...
using Elib2Ebook.ExternalServices.Freedlit.Types;
using HtmlAgilityPack;
using HtmlAgilityPack.CssSelectors.NetCore;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.Freedlit;
...
public class FreedlitGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private object GenerateAuthData()
    {
        return new
        {
            email = Config.Options.Login, password = Config.Options.Password, remember = false,
        };
    }
    ...
    {
        var response = await Config.Client.PostAsJsonAsync(
            SystemUrl.MakeRelativeUri("/api/bookpage/get-chapters"),
            new
            {
                book_id = book.Id
            });
    ...
    {
        var response = await Config.Client.PostAsJsonAsync(
            SystemUrl.MakeRelativeUri("/reader/get-content"),
            new
            {
