...
using Elib2Ebook.ExternalServices.IGram.Types;
using HtmlAgilityPack;
using HtmlAgilityPack.CssSelectors.NetCore;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.IGram;
...
public class IGramGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private FormUrlEncodedContent GenerateAuthData(string token)
    {
        ...
        {
            ["csrfmiddlewaretoken"] = token, ["remember"] = "checked", ["login"] = Config.Options.Login, ["password"] = Config.Options.Password
        };
