...
using Elib2Ebook.ExternalServices.MyBook.Types;
using HtmlAgilityPack;
using HtmlAgilityPack.CssSelectors.NetCore;
using Microsoft.Extensions.Logging;
using OAuth;

namespace Elib2Ebook.ExternalServices.MyBook;
...
public class MyBookGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    {
        Secret = "fYFPndYhOTW6YxIJ", Token = "f8d32ef906664ed3a1525b7298aac461"
    };
    ...
    public override async Task Authorize()
    {
        ...
        var payload = new
        {
            identifier = IDENTIFIER, email = Config.Options.Login, password = Config.Options.Password
        };
    ...
    private async Task<MyBookBook> GetDetails(Uri url)
    {
        ...
        {
            var bookId = details.Connected?.Id ?? details.MapFiles.FirstOrDefault(b => b.Book != 0)?.Book;
            if (bookId.HasValue)
            {
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        {
            Cover = await GetCover(details), Title = details.Name, Author = GetAuthor(details), Annotation = details.Annotation,
        };
