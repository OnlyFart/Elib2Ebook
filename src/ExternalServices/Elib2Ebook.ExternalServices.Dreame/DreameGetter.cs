...
using Elib2Ebook.ExternalServices.Dreame.Types;
using HtmlAgilityPack;
using HtmlAgilityPack.CssSelectors.NetCore;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.Dreame;
...
public class DreameGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    protected override string GetId(Uri url)
        => url.GetSegment(2).Split(".")[0].Split("-")[0];
    ...
    {
        var response = await Config.Client.GetFromJsonWithTriesAsync<DreameApiResponse<DreameChapter>>(
            _apiUrl.MakeRelativeUri($"/novel/getChapter?channel=dreamepmian-173&product=1&osType=2&cid={urlChapter.Id}&systemFlag=android&port=web"));
    ...
    {
        var response = await Config.Client.GetFromJsonWithTriesAsync<DreameApiResponse<DreameCatalog>>(
            _apiUrl.MakeRelativeUri($"novel/getcatelog?channel=dreamepmian-173&product=1&osType=2&nid={bookId}"));
