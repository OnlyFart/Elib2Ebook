...
namespace Elib2Ebook.ExternalServices.Libking;
...
public class LibkingGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private Author GetAuthor(HtmlDocument doc)
    {
        ...
        return a == null ? new Author("LibKing") : new Author(a.GetText(), SystemUrl.MakeRelativeUri(a.Attributes["href"].Value));
    }
    ...
    private async Task<List<Chapter>> FillChapters(HtmlDocument doc, Uri baseUrl, string name)
    {
        ...
        for (var url = GetFirstPage(doc, baseUrl); url != null;)
        {
        ...
        if (node != null)
        {
            while (node != null)
            {
                ...
                } while (node.NextSibling != null && !IsTitle(node.NextSibling));
