...
namespace Elib2Ebook.ExternalServices.Bookinist;
...
public class BookinistGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task<Book> Get(Uri url)
    {
        ...
        {
            Cover = await GetCover(doc, url), Chapters = await FillChapters(doc, url), Title = doc.GetTextBySelector("title"), Author = new Author("Bookinist")
        };
