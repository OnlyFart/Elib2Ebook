...
using Elib2Ebook.ExternalServices.BookRiver.Types;
using HtmlAgilityPack;
using HtmlAgilityPack.CssSelectors.NetCore;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.BookRiver;
...
public class BookriverGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    public override async Task Authorize()
    {
        ...
        var payload = new
        {
            email = Config.Options.Login, password = Config.Options.Password, rememberMe = 1
        };
    ...
    private static Seria GetSeria(HtmlDocument doc, Uri url)
    {
        ...
        return new Seria
        {
            Name = a.GetText()[5..].Trim(), Number = span.GetText().Trim().Trim('#'), Url = url.MakeRelativeUri(a.Attributes["href"].Value)
        };
    }
    ...
    private async Task<HtmlDocument> GetChapter(long bookChapterId)
    {
        ...
        return content.Deserialize<ApiResponse<ChapterContent>>().Data.Content.AsHtmlDoc();
    }
    ...
    private async Task<IEnumerable<BookRiverChapter>> GetToc(string bookId)
    {
        ...
        var content = await response.Content.ReadAsStringAsync();
        return SliceToc(content.Deserialize<ApiResponse<BookRiverChapter[]>>().Data, c => c.Name);
    }
