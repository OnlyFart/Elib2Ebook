...
using Elib2Ebook.ExternalServices.FanFicus.Types;
using HtmlAgilityPack;
using Microsoft.Extensions.Logging;

namespace Elib2Ebook.ExternalServices.FanFicus;
...
public class FanFicusGetter(BookGetterConfig config) : GetterBase(config)
{
    ...
    private object GenerateAuthData()
    {
        return new
        {
            username = Config.Options.Login, password = Config.Options.Password,
        };
    }
    ...
    private Author GetAuthor(FanFicusBook book)
    {
        ...
        return creator == null ? new Author("FanFicus") : new Author(creator.NickName, SystemUrl.MakeRelativeUri($"/user/{creator.Id}"));
    }
