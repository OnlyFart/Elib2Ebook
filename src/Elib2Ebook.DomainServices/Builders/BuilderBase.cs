...
public abstract class BuilderBase(Options options, ILogger logger)
{
    ...
    /// <returns></returns>
    protected virtual string GetFileName(Book book)
        => $"{GetTitle(book)}.{Extension}";
    ...
    /// <param name="book"></param>
    /// <returns></returns>
    protected virtual string GetTitle(Book book)
        => BookNameBuilder.Build(Options.BookNamePattern, book).Crop(100);
