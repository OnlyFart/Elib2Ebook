...
namespace Elib2Ebook.DomainServices.Extensions;
...
public static class UriExtension
{
    ...
    public static string GetSegment(this Uri self, int index)
    {
        ...
        {
            Scheme = self.Scheme, Host = self.Host, Path = Regex.Replace(self.AbsolutePath, "/+", "/")
        };
    ...
    public static bool IsSameSubDomain(this Uri self, Uri url)
    {
        ...
        return selfParts.Length >= urlParts.Length && string.Equals(
            Idn.GetAscii(string.Join(".", selfParts[^urlParts.Length..])),
            Idn.GetAscii(string.Join(".", urlParts)),
